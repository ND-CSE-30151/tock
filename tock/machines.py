...
class Store(syntax.String):
    ...
    def match(self, other):
        """Return ``True`` iff ``self`` (as a pattern) matches ``other`` (as a
        store). This relation is asymmetric: ``other`` is allowed
        to have symbols that are not found in ``self``."""
...
@dataclasses.dataclass(frozen=True, order=True)
class Configuration:
    """A configuration, which is essentially a tuple of ``Store`` objects.

    Arguments:
        arg (Configuration or str or sequence): Another ``Configuration`` to copy,
          a ``str`` to convert to a ``Configuration``, or a sequence of stores.
    """
    ...
    def match(self, other):
        """Return ``True`` iff ``self`` (as a pattern) matches ``other`` (as a
        configuration). This relation is asymmetric: ``other`` is allowed
        to have symbols that are not found in ``self``."""
...
class Transition:
    ...
    Arguments:
        ...
        transition (Transition or str): Another ``Transition`` to copy, or a
          ``str`` to convert to a ``Transition``.
    """
    ...
    def match(self, config):
        """Return ``True`` iff ``self`` can be applied to ``config``."""
...
class AlignedTransition:
    ...
    def __str__(self):
        return str(Transition(self.lhs, self.rhs))

    def _repr_html_(self):
        return Transition(self.lhs, self.rhs)._repr_html_()
...
class Machine:
    ...
    Arguments:
        ...
        store_types: A sequence of store types, one for each
    ...
    def edit(self):
        """Returns the drag-based editor for this Machine."""
    ...
    def has_stack(self, s):
        """Test whether store ``s`` is a stack, that is, it never moves from
        position 0.
        """
    ...
    def has_cell(self, s):
        """Test whether store ``s`` uses exactly one cell, and therefore can
    ...
    def has_input_stream(self, s):
        """Test whether store ``s`` is an input stream, that is, it only
    ...
    def has_tape(self, s):
        """Test whether store ``s`` is a tape, that is, it never inserts or
        deletes symbols."""
        for t in self.transitions:
    ...
    def has_readonly(self, s):
        """Test whether store ``s`` is read-only."""
        for t in self.transitions:
